RED_PRUEBA.py y RED_PRUEBA_2.py -> Redes neuronales más potentes la primera con una 0.6 de accuracy (modelo 1) y la segunda con un 0,8 (Sobre 1) (modelo 4).   

interfaz1.py -> Interfaz que hemos creado para hacer las pruebas del modelo.

interfaz2.py -> Interfaz que hemos creado de forma definitiva para implementar el modelo.

modelo_aves_pro.keras -> Modelo implementado con la interfaz 2.

modelo_prueba_1.keras -> Modelo que no pudimos usar debido a la restricción de tamaño del GitHub.

modelo_prueba_ultra_rapido.keras -> Modelo implementadocon la interfaz 2, fue el primero de los modelos que funcionó.

requirements.txt -> Archivo .txt con la lista de las librerías que el modelo requiere para su uso.
