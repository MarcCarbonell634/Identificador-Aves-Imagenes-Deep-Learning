...
st.title("🐦 Identificador de Especies de Aves (modo ultra rápido)")
st.markdown("Sube una imagen y el modelo hará una predicción rápida.")
st.divider()
...
def load_model_cached():
    if not os.path.exists(MODEL_PATH):
        st.error(f"❌ No se encuentra el modelo: {MODEL_PATH}")
        st.stop()
